...
{
  control_mode_.set(control_mode_type::FAST);
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
      ...
      std::shared_ptr<ControllerModeSrvType::Response> response)
  {
    if (request->data)
    {
      control_mode_.set(control_mode_type::SLOW);
    }
    else
    {
      control_mode_.set(control_mode_type::FAST);
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  const auto control_mode_op = control_mode_.try_get();
  if (control_mode_op.has_value() && control_mode_op.value() == control_mode_type::SLOW)
  {
    fast_factor = 1.0;
  }
